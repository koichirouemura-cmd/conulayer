...
**AIが動かすシステムの無駄を根本から削ることで、エネルギー消費の増加を抑えられるのではないか？。**
...
```
Linuxカーネルアップデート（現在）:
  人間の開発者が改善 → 人間のメンテナーがレビュー → 全ユーザーが恩恵

AI-nativeプラットフォーム共有（構想）:
  AIがプラットフォームを構築・改善
      ↓
...
信頼の根拠も変わる：「誰が作ったか」ではなく「動くことが証明されているか」。

### コンポーネントレジストリ

**「作る前に探す」をAI開発のデフォルトにする。**

現状はローカルのAlpine（10.0.2.2:8888）からWASMを取得するだけ。
将来は外部レジストリから検索・取得できる仕組みを構築する。

```
現状: unikernel起動 → Alpine:8888 からダウンロード（ローカルのみ）
構想: unikernel起動 → レジストリを検索 → 既存コンポーネントを取得
```

### MCPサーバー統合

Claude CodeとこのOSシステムを繋ぐ接触点をMCPサーバーとする。
...
SSHもDockerも不要。意図を伝えるだけでアプリができる体験を目指す。

### Personal Management OS（完全版）

現状はAlpine Linux + vsockの最小実装。構想では：
- インターネット非接続・ローカルのみ
- TPMによるデバイス結合
- 期限付き能力トークンによるアクセス制御
- 全アクセスの監査ログ
